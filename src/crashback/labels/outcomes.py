...
def build_labels(
    ...
    *,
    ...
    fwd = (
        ...
        .join(
            prices_seq.select("security_id", "seq", "close", "high"),
    ...
    for h in horizons:
        determined = pl.col("is_delisted") | (pl.col(nforward_col(h)) >= h)
        censored = ~determined
    ...
    keep = ["event_id", "security_id", "crash_date", "p0", "delisting_return_missing"]
    for h in horizons:
